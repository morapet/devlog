...
- **Trusted-localhost auth** (`DEVLOG_AUTH` / `DEVLOG_AUTH_TOKEN`, spec §4.11) — loopback is trusted; remote requests need a shared secret. HMAC-signed 30-day session cookie via a JS login overlay for browsers, `Authorization: Bearer <token>` for API clients/MCP. `DEVLOG_AUTH=off` = the old no-auth localhost behavior.
- **Hosting recipes** (`deploy/`) — Cloudflare Tunnel (free, no open ports), VPS + Caddy (auto-TLS), PythonAnywhere (free tier, WSGI bridge via `a2wsgi`), Cloud Run (Litestream → GCS for SQLite persistence, max 1 instance).
